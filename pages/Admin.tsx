...
const Admin: React.FC<AdminProps> = ({ opTypes, setOpTypes, operatives, users, setUsers, currentUserRole, coloniaCatalog, setColoniaCatalog }) => {
  ...
  const addUser = async (e: React.FormEvent) => {
    ...
    const userToAdd: User = {
      id: Math.random().toString(36).substring(2, 11), // For the local state, DB will generate its own if UUID
      fullName: removeAccents(newUser.fullName!),
      username: newUser.username!,
      password: newUser.password!,
      role: newUser.role!,
      assignedRegion: ['ADMIN', 'DIRECTOR', 'ANALISTA'].includes(newUser.role!) ? null : newUser.assignedRegion,
  ...
  const saveEditUser = async () => {
    ...
    const updates = { ...newUser as User, fullName: removeAccents(newUser.fullName || '') };
  ...
  // Colony Management
  const addColony = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newColony.colony || !newColony.quadrant) return;
    const item = { ...newColony, colony: removeAccents(newColony.colony) };
    const { error } = await supabase.from('colonia_catalog').insert([item]);
    if (!error) {
      setColoniaCatalog([...coloniaCatalog, item]);
      setNewColony({ ...newColony, region: REGIONS[0], quadrant: '', colony: '' });
    } else {
      alert('Error al agregar colonia: ' + error.message);
    }
  };

  const removeColony = async (col: CatalogEntry) => {
    if (!confirm('¿ELIMINAR ESTA COLONIA DEL CATALOGO?')) return;
    const { error } = await supabase.from('colonia_catalog')
      .delete()
      .eq('colony', col.colony)
      .eq('quadrant', col.quadrant)
      .eq('region', col.region);

    if (!error) {
      setColoniaCatalog(coloniaCatalog.filter(c => !(c.colony === col.colony && c.quadrant === col.quadrant && c.region === col.region)));
    } else {
      alert('Error al eliminar colonia: ' + error.message);
    }
  };
  ...
  const addType = async () => {
    if (newType && !opTypes.includes(removeAccents(newType))) {
      const name = removeAccents(newType);
      const { error } = await supabase.from('operative_types').insert([{ name }]);
      if (!error) {
        setOpTypes([...opTypes, name]);
        setNewType("");
      } else {
  ...
  const removeType = async (index: number) => {
    const name = opTypes[index];
    if (!confirm(`¿ELIMINAR EL TIPO "${name}"?`)) return;
    const { error } = await supabase.from('operative_types').delete().eq('name', name);
    if (!error) {
